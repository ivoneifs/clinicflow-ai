...
import { Activity, ArrowUpRight, Bell, CalendarDays, Check, ChevronDown, Clock3, FileText, LayoutDashboard, LogOut, MessageCircle, MoreHorizontal, Plus, Search, Settings2, Sparkles, Stethoscope, Users, Video } from 'lucide-react';
...
type Conversation = { id: string; patient: { name: string; phone?: string } | null; messageText: string | null; direction?: 'RECEIVED' | 'SENT'; createdAt: string };
type ConversationFilter = 'all' | 'unread' | 'ai';
type ConversationForm = { patientName: string; phone: string; messageText: string };
type HoursForm = { startsAt: string; endsAt: string };
...
type SessionUser = { id?: string; name?: string; email?: string; role?: string; specialty?: string | null; clinic?: { id?: string; name?: string; slug?: string } | null };
...
function roleLabel(role?: string) { return role === 'MASTER' ? 'Master' : role === 'ADMIN' ? 'Administrador' : role === 'DOCTOR' ? 'Médico' : role === 'ATTENDANT' ? 'Atendente' : 'Profissional'; }
...
function AppointmentRows({ appointments, checkedIn, onCheckIn }: { appointments: Appointment[]; checkedIn: string[]; onCheckIn: (id: string) => void }) {
  ...
  return <div className="agenda-list">{appointments.map((item) => { const tone = appointmentTone(item.status); return <div className="appointment-row" key={item.id}><div className="appointment-time">{formatTime(item.startsAt)}</div><div className={`appointment-status-line ${tone}`} /><div className="appointment-person"><div className={`avatar avatar-${tone}`}>{initials(item.patient.name)}</div><div><strong>{item.patient.name}</strong><span>Consulta · {item.doctor.specialty || 'Clínica geral'}</span></div></div><div className="appointment-doctor"><span>Profissional</span><strong>{item.doctor.name}</strong></div><div className="appointment-action">{item.status === 'IN_PROGRESS' ? <button className="checkin-button" onClick={() => onCheckIn(item.id)}>{checkedIn.includes(item.id) ? <><Check size={14} /> Na sala</> : 'Check-in'}</button> : <StatusPill tone={tone}>{appointmentLabel(item.status)}</StatusPill>}<span className="dots" aria-hidden="true"><MoreHorizontal size={17} /></span></div></div>; })}</div>;
}

function ConversationList({ conversations, selectedId, onSelect, unreadIds }: { conversations: Conversation[]; selectedId?: string; onSelect: (id: string) => void; unreadIds?: Set<string> }) {
  const [hydrated, setHydrated] = useState(false);
  useEffect(() => setHydrated(true), []);
  return <div className="conversation-list">{conversations.length === 0 ? <div className="empty-state">Nenhuma conversa encontrada.</div> : conversations.map((conversation, index) => <button type="button" className={`conversation ${conversation.id === selectedId ? 'conversation-selected' : ''}`} key={conversation.id} onClick={() => onSelect(conversation.id)}><div className={`avatar avatar-${['coral', 'violet', 'green'][index % 3]}`}>{initials(conversation.patient?.name || 'Paciente')}</div><div className="conversation-copy"><div><strong>{conversation.patient?.name || 'Paciente'}</strong>{unreadIds?.has(conversation.id) && <span className="unread-dot" />}</div><p>{conversation.messageText || 'Mensagem sem texto'}</p></div><time>{hydrated ? relativeTime(conversation.createdAt) : '—'}</time></button>)}</div>;
}

function Overview({ data, appointments, conversations, loading, displayDate, displayName, checkedIn, onCheckIn, onNewAppointment = () => undefined, onOpenSection = () => undefined }: { data: DashboardData; appointments: Appointment[]; conversations: Conversation[]; loading: boolean; displayDate: string; displayName: string; checkedIn: string[]; onCheckIn: (id: string) => void; onNewAppointment?: () => void; onOpenSection?: (section: string) => void }) {
  return <><div className="welcome-row"><div><p className="eyebrow">{displayDate.toUpperCase()}</p><h1>Bom dia, {displayName} <span>✦</span></h1><p className="subheading">Aqui está o pulso da sua clínica hoje.</p></div><button className="primary-button" onClick={onNewAppointment}><Plus size={17} />Novo agendamento</button></div>
    ...
    <div className="dashboard-grid"><div className="panel agenda-panel"><div className="panel-header"><div><h2>Agenda de hoje</h2><p>{displayDate.split(' de ')[0]} <span>·</span> {data.appointments.length} consultas</p></div><button className="ghost-button" onClick={() => onOpenSection('Agenda')}>Ver agenda completa <ArrowUpRight size={15} /></button></div><AppointmentRows appointments={appointments} checkedIn={checkedIn} onCheckIn={onCheckIn} /></div><div className="panel inbox-panel"><div className="panel-header"><div><h2>Atendimento</h2><p>WhatsApp <span className="live-dot" /> ao vivo</p></div><button className="circle-add" aria-label="Nova conversa" onClick={() => onOpenSection('Atendimento')}><Plus size={17} /></button></div><div className="inbox-tabs"><button type="button" className="tab-active" onClick={() => onOpenSection('Atendimento')}>Todas <b>{data.conversations.length}</b></button><button type="button" onClick={() => onOpenSection('Atendimento')}>Não lidas <b>{Math.min(data.conversations.length, 4)}</b></button></div><ConversationList conversations={conversations} onSelect={() => onOpenSection('Atendimento')} /><button className="inbox-footer" onClick={() => onOpenSection('Atendimento')}>Abrir central de atendimento <ArrowUpRight size={15} /></button></div></div>
    <div className="bottom-grid"><div className="panel ai-panel"><div className="ai-orb"><Sparkles size={19} /></div><div><p className="eyebrow">ASSISTENTE CLINICFLOW</p><h2>A clínica não para quando você sai.</h2><p>A IA acolheu <strong>64 conversas</strong> esta semana e converteu <strong>18 agendamentos</strong> automaticamente.</p></div><div className="ai-chart"><span style={{ height: '38%' }} /><span style={{ height: '54%' }} /><span style={{ height: '44%' }} /><span style={{ height: '78%' }} /><span style={{ height: '62%' }} /><span style={{ height: '92%' }} /></div></div><div className="panel tele-panel"><div className="tele-heading"><div className="tele-icon"><Video size={18} /></div><div><h2>Próxima teleconsulta</h2><p>Começa em 01h 18min</p></div></div><div className="tele-person"><div className="avatar avatar-blue">LR</div><div><strong>Lucas Ribeiro</strong><span>Clínica geral · 11:30</span></div><span className="status status-blue"><span className="status-dot" />sala não configurada</span></div></div></div></>;
}

function SectionScreen({ section, data, appointments, conversations, checkedIn, onCheckIn, selectedConversationId, onSelectConversation, aiEnabled, onToggleAi, aiSaving, suggestion, suggesting, onSuggestReply, evolutionStatus, patientRecords, team, currentUser, onNewPatient, onNewAppointment, onNewConversation, onOpenRecord, hoursForm, onEditHours, conversationFilter, onConversationFilterChange, unreadConversationIds, replyText, onReplyTextChange, sendingReply, sendError, onSendReply }: { section: string; data: DashboardData; appointments: Appointment[]; conversations: Conversation[]; checkedIn: string[]; onCheckIn: (id: string) => void; selectedConversationId?: string; onSelectConversation: (id: string) => void; aiEnabled: boolean; onToggleAi: () => void; aiSaving: boolean; suggestion: string; suggesting: boolean; onSuggestReply: (conversation?: Conversation) => void; evolutionStatus?: IntegrationStatus; patientRecords: PatientSummary[]; team: TeamMember[]; currentUser: SessionUser | null; onNewPatient: () => void; onNewAppointment?: () => void; onNewConversation: () => void; onOpenRecord: (patient: PatientSummary) => void; hoursForm: HoursForm; onEditHours: () => void; conversationFilter: ConversationFilter; onConversationFilterChange: (value: ConversationFilter) => void; unreadConversationIds: Set<string>; replyText: string; onReplyTextChange: (value: string) => void; sendingReply: boolean; sendError: string; onSendReply: () => void }) {
  ...
  const people = team.length ? team : currentUser?.name ? [{ id: currentUser.id || 'current-user', name: currentUser.name, specialty: currentUser.specialty || null, role: currentUser.role || 'PROFESSIONAL' }] : [];
  const handleExportPatients = () => {
    const csv = [['Paciente', 'Telefone', 'Último contato', 'Registros'], ...patients.map((patient) => [patient.name, patient.phone, patient.lastVisit, String(patient.recordsCount || 0)])]
      .map((row) => row.map((value) => `"${value.replace(/"/g, '""')}"`).join(';')).join('\n');
    const link = document.createElement('a');
    link.href = URL.createObjectURL(new Blob([`\uFEFF${csv}`], { type: 'text/csv;charset=utf-8' }));
    link.download = 'clinicflow-pacientes.csv';
    link.click();
  ...
  if (section === 'Atendimento') { const selected = conversations.find((item) => item.id === selectedConversationId) || conversations[0]; const filteredConversations = conversationFilter === 'unread' ? conversations.filter((item) => unreadConversationIds.has(item.id)) : conversationFilter === 'ai' ? conversations.filter((item) => item.direction === 'SENT') : conversations; const unreadCount = conversations.filter((item) => unreadConversationIds.has(item.id)).length; const aiCount = conversations.filter((item) => item.direction === 'SENT').length; return <><SectionIntro eyebrow="CANAIS · WHATSAPP" title="Central de atendimento" description="Acompanhe as conversas e deixe a IA cuidar do primeiro acolhimento." action="Nova conversa" onAction={onNewConversation} /><div className={`ai-auto-control ${aiEnabled ? 'ai-auto-control-on' : 'ai-auto-control-off'}`}><div className="ai-auto-icon"><Sparkles size={17} /></div><div className="ai-auto-copy"><strong>Atendente virtual autônoma</strong><span>{aiEnabled ? 'Ativa: responde texto, áudio, documentos e imagens; devolve áudio quando o agente gerar voz.' : 'Pausada: a equipe assume as respostas manualmente.'}</span></div><div className="ai-auto-state"><small>{aiEnabled ? 'EM OPERAÇÃO' : 'PAUSADA'}</small><button type="button" className={`toggle toggle-large ${aiEnabled ? 'toggle-on' : ''}`} onClick={onToggleAi} disabled={aiSaving} aria-pressed={aiEnabled} aria-label={aiEnabled ? 'Pausar respostas automáticas da IA' : 'Ativar respostas automáticas da IA'}><span /></button></div></div><div className="inbox-screen-grid"><div className="panel inbox-full-panel"><div className="inbox-tabs"><button type="button" className={conversationFilter === 'all' ? 'tab-active' : ''} onClick={() => onConversationFilterChange('all')}>Todas <b>{conversations.length}</b></button><button type="button" className={conversationFilter === 'unread' ? 'tab-active' : ''} onClick={() => onConversationFilterChange('unread')}>Não lidas <b>{unreadCount}</b></button><button type="button" className={conversationFilter === 'ai' ? 'tab-active' : ''} onClick={() => onConversationFilterChange('ai')}>Com IA <b>{aiCount}</b></button></div><ConversationList conversations={filteredConversations} selectedId={selected?.id} onSelect={onSelectConversation} unreadIds={unreadConversationIds} /></div><div className="panel conversation-detail"><div className="detail-top"><div className="avatar avatar-coral">{initials(selected?.patient?.name || 'Paciente')}</div><div><h2>{selected?.patient?.name || 'Selecione uma conversa'}</h2><p>WhatsApp · {selected ? relativeTime(selected.createdAt) : 'sem mensagens'}</p></div><span className="live-badge">online</span></div><div className="message-bubble">{selected?.messageText || 'Escolha uma conversa para visualizar o histórico.'}</div>{suggestion && <div className="suggestion-bubble"><Sparkles size={13} /><span>{suggestion}</span></div>}<div className="reply-box"><textarea aria-label="Resposta" value={replyText} onChange={(event) => onReplyTextChange(event.target.value)} placeholder={selected ? 'Escreva uma resposta...' : 'Selecione uma conversa para responder'} disabled={!selected || sendingReply} /><div className="reply-actions"><button type="button" className="ai-reply-button" onClick={() => onSuggestReply(selected)} disabled={!selected || suggesting}><Sparkles size={14} />{suggesting ? 'Gerando...' : 'Sugerir com IA'}</button><button type="button" className="send-button" onClick={onSendReply} disabled={!selected || !replyText.trim() || sendingReply} aria-label="Enviar resposta"><ArrowUpRight size={15} /></button></div>{sendError && <p className="form-warning">{sendError}</p>}</div></div></div></>; }
  if (section === 'Pacientes') return <><SectionIntro eyebrow="RELACIONAMENTO · PACIENTES" title="Pacientes" description="Uma visão rápida das pessoas que movimentam a clínica." action="Cadastrar paciente" onAction={onNewPatient} /><div className="panel table-panel"><div className="table-toolbar"><strong>{patients.length} pacientes recentes</strong><button type="button" className="ghost-button" onClick={handleExportPatients}>Exportar lista <ArrowUpRight size={14} /></button></div><div className="patient-table"><div className="table-row table-head"><span>Paciente</span><span>Telefone</span><span>Último contato</span><span>Status</span></div>{patients.length ? patients.map((patient, index) => <div className="table-row" key={patient.id || patient.name}><span className="table-person"><span className={`avatar avatar-${['coral', 'mint', 'violet'][index % 3]}`}>{initials(patient.name)}</span><strong>{patient.name}</strong></span><span>{patient.phone}</span><span>{patient.lastVisit}</span><StatusPill tone={index % 2 ? 'mint' : 'blue'}>{patient.recordsCount ? `${patient.recordsCount} registro${patient.recordsCount > 1 ? 's' : ''}` : index % 2 ? 'ativo' : 'retorno hoje'}</StatusPill></div>) : <div className="empty-state">Nenhum paciente recente.</div>}</div></div></>;
  if (section === 'Prontuários') return <><SectionIntro eyebrow="CUIDADO · HISTÓRICO CLÍNICO" title="Prontuários" description="Acesse o contexto certo antes de cada atendimento." /><div className="record-grid">{patients.map((patient, index) => <div className="panel record-card" key={patient.name}><div className="record-heading"><div className={`avatar avatar-${['coral', 'mint', 'violet'][index % 3]}`}>{initials(patient.name)}</div><div><h2>{patient.name}</h2><p>Último atendimento {patient.lastVisit}</p></div><MoreHorizontal size={16} /></div><div className="record-line"><FileText size={14} /><span>{index % 2 ? 'Anamnese e evolução' : 'Retorno · documentação completa'}</span></div><button type="button" className="ghost-button" onClick={() => onOpenRecord(patient)}>Abrir prontuário <ArrowUpRight size={14} /></button></div>)}</div></>;
  if (section === 'Equipe') return <><SectionIntro eyebrow="GESTÃO · PROFISSIONAIS" title="Equipe clínica" description="Veja quem está online e como as agendas estão distribuídas." /><div className="team-grid">{people.map((person, index) => { const tone = person.role === 'ADMIN' || person.role === 'MASTER' ? 'lime' : ['blue', 'violet', 'green'][index % 3]; return <div className="panel team-card" key={person.id || person.name}><div className={`avatar avatar-${tone}`}>{initials(person.name)}</div><div><h2>{person.name}</h2><p>{person.specialty || roleLabel(person.role)}</p></div><span className="online-indicator" /></div>; })}{!people.length && <div className="empty-state">Nenhum profissional cadastrado.</div>}</div></>;
  return <><SectionIntro eyebrow="CONFIGURAÇÕES · CLINICFLOW" title="Configurações" description="Ajuste a operação da clínica e o comportamento da assistente." /><div className="settings-list"><div className="panel setting-row"><div className="setting-icon"><Sparkles size={17} /></div><div><h2>Assistente IA</h2><p>Responde novas conversas fora do horário comercial.</p></div><button className={`toggle ${aiEnabled ? 'toggle-on' : ''}`} onClick={onToggleAi} aria-label="Alternar assistente IA"><span /></button></div><div className="panel setting-row"><div className="setting-icon setting-blue"><Clock3 size={17} /></div><div><h2>Horário de funcionamento</h2><p>Segunda a sexta · {hoursForm.startsAt} às {hoursForm.endsAt}</p></div><button type="button" className="ghost-button" onClick={onEditHours}>Editar <ArrowUpRight size={14} /></button></div><div className="panel setting-row"><div className="setting-icon setting-coral"><MessageCircle size={17} /></div><div><h2>WhatsApp conectado</h2><p>Instância {evolutionStatus?.instance || 'nova'} · {evolutionStatus?.connected ? 'sincronizada agora' : evolutionStatus?.status === 'not_found' ? 'instância não encontrada' : 'verificando conexão'}</p></div>{evolutionStatus?.connected ? <StatusPill tone="mint">conectado</StatusPill> : <StatusPill tone="amber">{evolutionStatus?.status || 'verificando'}</StatusPill>}</div></div></>;
}

function ConversationModal({ form, setForm, saving, error, onClose, onSubmit }: { form: ConversationForm; setForm: (value: ConversationForm) => void; saving: boolean; error: string; onClose: () => void; onSubmit: () => void }) {
  return <div className="modal-backdrop" role="presentation"><form className="modal-card" onSubmit={(event) => { event.preventDefault(); onSubmit(); }}><div className="modal-heading"><div><p className="eyebrow">WHATSAPP · NOVA CONVERSA</p><h2>Iniciar conversa</h2></div><button type="button" className="dots" onClick={onClose} aria-label="Fechar">×</button></div><label>Nome do paciente<input required value={form.patientName} onChange={(event) => setForm({ ...form, patientName: event.target.value })} /></label><label>Telefone<input required value={form.phone} onChange={(event) => setForm({ ...form, phone: event.target.value })} placeholder="(11) 99999-9999" /></label><label>Mensagem<textarea required value={form.messageText} onChange={(event) => setForm({ ...form, messageText: event.target.value })} placeholder="Digite a primeira mensagem..." /></label>{error && <p className="form-warning">{error}</p>}<div className="modal-actions"><button type="button" className="ghost-button" onClick={onClose}>Cancelar</button><button type="submit" className="primary-button" disabled={saving}>{saving ? 'Enviando...' : 'Enviar mensagem'}</button></div></form></div>;
}

function RecordModal({ patient, onClose }: { patient: PatientSummary; onClose: () => void }) {
  return <div className="modal-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }}><section className="modal-card record-modal" role="dialog" aria-modal="true" aria-labelledby="record-modal-title"><div className="modal-heading"><div><p className="eyebrow">CUIDADO · PRONTUÁRIO</p><h2 id="record-modal-title">{patient.name}</h2></div><button type="button" className="dots" onClick={onClose} aria-label="Fechar">×</button></div><div className="record-modal-person"><div className="avatar avatar-coral">{initials(patient.name)}</div><div><strong>{patient.phone}</strong><span>Paciente cadastrado</span></div></div><div className="record-modal-grid"><div><span>Último atendimento</span><strong>{patient.lastVisit}</strong></div><div><span>Registros clínicos</span><strong>{patient.recordsCount || 0}</strong></div></div><div className="record-modal-note"><FileText size={16} /><div><strong>Histórico do paciente</strong><span>{patient.recordsCount ? 'Os registros deste paciente estão disponíveis para consulta.' : 'Ainda não há registros clínicos cadastrados.'}</span></div></div><div className="modal-actions"><button type="button" className="primary-button" onClick={onClose}>Fechar prontuário</button></div></section></div>;
}

function HoursModal({ form, setForm, onClose, onSubmit }: { form: HoursForm; setForm: (value: HoursForm) => void; onClose: () => void; onSubmit: () => void }) {
  return <div className="modal-backdrop" role="presentation"><form className="modal-card" onSubmit={(event) => { event.preventDefault(); onSubmit(); }}><div className="modal-heading"><div><p className="eyebrow">CONFIGURAÇÕES · HORÁRIOS</p><h2>Horário de funcionamento</h2></div><button type="button" className="dots" onClick={onClose} aria-label="Fechar">×</button></div><div className="form-grid"><label>Abre às<input required type="time" value={form.startsAt} onChange={(event) => setForm({ ...form, startsAt: event.target.value })} /></label><label>Fecha às<input required type="time" value={form.endsAt} onChange={(event) => setForm({ ...form, endsAt: event.target.value })} /></label></div><div className="modal-actions"><button type="button" className="ghost-button" onClick={onClose}>Cancelar</button><button type="submit" className="primary-button">Salvar horário</button></div></form></div>;
}
...
export default function Home() {
  ...
  const [conversationForm, setConversationForm] = useState<ConversationForm>({ patientName: '', phone: '', messageText: '' });
  const [recordPatient, setRecordPatient] = useState<PatientSummary | null>(null);
  const [showHoursForm, setShowHoursForm] = useState(false);
  const [hoursForm, setHoursForm] = useState<HoursForm>({ startsAt: '08:00', endsAt: '18:00' });

  useEffect(() => {
    if (!sessionStorage.getItem('clinicflow_user_token')) { window.location.replace('/login'); return; }
    try { const savedUser = sessionStorage.getItem('clinicflow_user'); if (savedUser) setCurrentUser(JSON.parse(savedUser) as SessionUser); } catch { setCurrentUser(null); }
    setAuthReady(true);
  }, []);
  ...
  useEffect(() => {
    if (!clinicId) return;
    let cancelled = false;
    const load = async () => { try { const response = await fetch(`${apiUrl}/api/v1/dashboard/summary?clinicId=${encodeURIComponent(clinicId)}`, { cache: 'no-store', headers: authHeaders() }); if (!response.ok) throw new Error(`summary-${response.status}`); const next = await response.json() as DashboardData; if (!cancelled) { setData((current) => ({ ...next, conversations: current.conversations })); setSyncError(false); } } catch { if (!cancelled) setSyncError(true); } finally { if (!cancelled) setLoading(false); } };
    void load(); const timer = window.setInterval(load, 30000); return () => { cancelled = true; window.clearInterval(timer); };
  }, []);

  useEffect(() => {
    try {
      const savedHours = localStorage.getItem('clinicflow_hours');
      if (savedHours) setHoursForm(JSON.parse(savedHours) as HoursForm);
    } catch { /* mantém o horário padrão */ }
  }, []);
  ...
  useEffect(() => {
    ...
    const loadAiSettings = async () => {
      try {
        const response = await fetch(`${apiUrl}/api/v1/settings/ai?clinicId=${encodeURIComponent(clinicId)}`, { cache: 'no-store', headers: authHeaders() });
        if (!response.ok) throw new Error('ai-settings');
        const body = await response.json() as { enabled?: boolean };
        if (!cancelled && typeof body.enabled === 'boolean') setAiEnabled(body.enabled);
      } catch { /* mantém o padrão ativo até a API responder */ }
    };
    void loadAiSettings();
  ...
  useEffect(() => {
    ...
    const loadConversations = async () => {
      try {
        const response = await fetch(`${apiUrl}/api/v1/conversations?clinicId=${encodeURIComponent(clinicId)}&limit=30`, { cache: 'no-store', headers: authHeaders() });
        if (!response.ok) throw new Error(`conversations-${response.status}`);
        const next = await response.json() as Conversation[];
        if (!cancelled) setData((current) => ({ ...current, conversations: next }));
      } catch {
  ...
  const clinicName = currentUser?.clinic?.name || 'Clínica Vitta';
  ...
  const handleNewConversation = () => { setConversationError(''); setConversationForm({ patientName: '', phone: '', messageText: '' }); setShowConversationForm(true); };
  const handleCreateConversation = async () => {
    if (!clinicId || !conversationForm.patientName.trim() || !conversationForm.phone.trim() || !conversationForm.messageText.trim()) return;
    setSavingConversation(true); setConversationError('');
    try {
      const response = await fetch(`${apiUrl}/api/v1/conversations`, { method: 'POST', headers: authHeaders({ 'content-type': 'application/json' }), body: JSON.stringify({ clinicId, ...conversationForm }) });
      const body = await response.json() as Conversation & { error?: string };
      if (!response.ok) throw new Error(body.error || 'Não foi possível iniciar a conversa.');
      setData((current) => ({ ...current, conversations: [body, ...current.conversations.filter((item) => item.id !== body.id)].slice(0, 30) }));
      setSelectedConversationId(body.id);
      setShowConversationForm(false);
    } catch (error) { setConversationError(error instanceof Error ? error.message : 'Não foi possível iniciar a conversa.'); } finally { setSavingConversation(false); }
  };
  const handleSelectConversation = (id: string) => { setSelectedConversationId(id); setReadConversationIds((current) => new Set(current).add(id)); setReplyText(''); setSuggestion(''); setSendError(''); };
  const handleSendReply = async () => {
    const selected = conversations.find((item) => item.id === selectedConversationId) || conversations[0];
    if (!clinicId || !selected?.patient?.phone || !replyText.trim()) return;
    setSendingReply(true); setSendError('');
    try {
      const response = await fetch(`${apiUrl}/api/v1/conversations`, { method: 'POST', headers: authHeaders({ 'content-type': 'application/json' }), body: JSON.stringify({ clinicId, patientName: selected.patient.name, phone: selected.patient.phone, messageText: replyText.trim() }) });
      const body = await response.json() as Conversation & { error?: string };
      if (!response.ok) throw new Error(body.error || 'Não foi possível enviar a resposta.');
      setData((current) => ({ ...current, conversations: [body, ...current.conversations.filter((item) => item.id !== body.id)].slice(0, 30) }));
      setReplyText(''); setSuggestion('');
    } catch (error) { setSendError(error instanceof Error ? error.message : 'Não foi possível enviar a resposta.'); } finally { setSendingReply(false); }
  };
  const handleSuggestReply = async (conversation?: Conversation) => {
    ...
    try { const response = await fetch(`${apiUrl}/api/v1/assistant/suggest-reply`, { method: 'POST', headers: authHeaders({ 'content-type': 'application/json' }), body: JSON.stringify({ patientName: conversation.patient.name, messageText: conversation.messageText }) }); const body = await response.json() as { suggestion?: string; error?: string }; if (!response.ok) throw new Error(body.error || 'Não foi possível gerar a sugestão.'); setSuggestion(body.suggestion || ''); setReplyText(body.suggestion || ''); } catch (error) { setSuggestion(error instanceof Error ? error.message : 'Não foi possível gerar a sugestão.'); } finally { setSuggesting(false); }
  };
  const handleToggleAi = async () => {
    ...
    setAiSaving(true);
    try {
      const response = await fetch(`${apiUrl}/api/v1/settings/ai`, { method: 'PATCH', headers: authHeaders({ 'content-type': 'application/json' }), body: JSON.stringify({ clinicId, enabled: next }) });
      if (!response.ok) throw new Error('ai-settings');
    } catch {
      setAiEnabled(!next);
  ...
  const handleSaveHours = () => {
    localStorage.setItem('clinicflow_hours', JSON.stringify(hoursForm));
    setShowHoursForm(false);
  };
  const handleLogout = () => { sessionStorage.removeItem('clinicflow_user_token'); sessionStorage.removeItem('clinicflow_user'); window.location.replace('/login'); };
  ...
  return <main className="shell"><aside className="sidebar"><div className="brand"><span className="brand-mark"><Activity size={18} strokeWidth={2.7} /></span><span>clinic<span>flow</span></span><span className="brand-beta">AI</span></div><div className="workspace-switcher"><div className="clinic-avatar">{initials(clinicName)}</div><div><strong>{clinicName}</strong><small>{currentUser?.clinic?.slug || 'Unidade principal'}</small></div><ChevronDown size={15} /></div><nav><p className="nav-label">Workspace</p>{[[LayoutDashboard, 'Visão geral'], [MessageCircle, 'Atendimento', '12'], [CalendarDays, 'Agenda'], [Users, 'Pacientes'], [FileText, 'Prontuários']].map(([Icon, label, count]: any) => <button className={section === label ? 'nav-item active' : 'nav-item'} onClick={() => { setSection(label); setSuggestion(''); }} key={label}><Icon size={18} />{label}{count && <span className="nav-count">{count}</span>}</button>)}<p className="nav-label space-top">Gestão</p>{[[Stethoscope, 'Equipe'], [Settings2, 'Configurações']].map(([Icon, label]: any) => <button className={section === label ? 'nav-item active' : 'nav-item'} onClick={() => setSection(label)} key={label}>{<Icon size={18} />}{label}</button>)}</nav><div className="sidebar-bottom"><div className={`pulse-card ${aiEnabled ? '' : 'pulse-card-off'}`}><div className="pulse-icon"><Sparkles size={15} /></div><div><strong>IA {aiEnabled ? 'em operação' : 'pausada'}</strong><span>{aiEnabled ? 'Atendimento 24/7 ativo' : 'Respostas manuais ativas'}</span></div><i /></div><div className="user-chip"><div className="avatar avatar-lime">{initials(displayName)}</div><div><strong>{displayName}</strong><small>{displayRole}</small></div><button className="logout-button" onClick={handleLogout} aria-label="Sair" title="Sair"><LogOut size={16} /></button></div></div></aside><section className="content"><header className="topbar"><div className="crumb"><span>Workspace</span><b>/</b><strong>{section}</strong></div><div className="top-actions"><div className="search"><Search size={17} /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Buscar paciente, conversa..." /></div><button className="icon-button notification" onClick={() => setShowNotifications(!showNotifications)}><Bell size={19} /><span /></button>{showNotifications && <div className="notification-pop"><strong>Notificações</strong><p>{data.conversations.length} conversas carregadas no painel.</p><p>{data.waiting} paciente{data.waiting === 1 ? '' : 's'} aguardando atendimento.</p></div>}<div className="avatar avatar-coral">{initials(displayName)}</div></div></header><div className="main-scroll">{section === 'Visão geral' ? <Overview data={data} appointments={appointments} conversations={conversations} loading={loading} displayDate={displayDate} displayName={displayName} checkedIn={checkedIn} onCheckIn={handleCheckIn} onNewAppointment={() => setShowAppointmentForm(true)} onOpenSection={setSection} /> : <SectionScreen section={section} data={data} appointments={appointments} conversations={conversations} checkedIn={checkedIn} onCheckIn={handleCheckIn} selectedConversationId={selectedConversationId} onSelectConversation={handleSelectConversation} aiEnabled={aiEnabled} onToggleAi={handleToggleAi} aiSaving={aiSaving} suggestion={suggestion} suggesting={suggesting} onSuggestReply={handleSuggestReply} evolutionStatus={evolutionStatus} patientRecords={patientRecords} team={team} currentUser={currentUser} onNewPatient={() => setShowPatientForm(true)} onNewAppointment={() => setShowAppointmentForm(true)} onNewConversation={handleNewConversation} onOpenRecord={(patient) => setRecordPatient(patient)} hoursForm={hoursForm} onEditHours={() => setShowHoursForm(true)} conversationFilter={conversationFilter} onConversationFilterChange={setConversationFilter} unreadConversationIds={new Set(conversations.filter((item) => item.direction !== 'SENT' && !readConversationIds.has(item.id)).map((item) => item.id))} replyText={replyText} onReplyTextChange={setReplyText} sendingReply={sendingReply} sendError={sendError} onSendReply={() => { void handleSendReply(); }} />}{syncError && <p className="sync-note">API indisponível; exibindo os últimos dados de demonstração. Configure o banco e recarregue para sincronizar.</p>}<footer><span>ClinicFlow AI · Operação clínica em um só lugar</span><a href="https://deerflow.tech" target="_blank" rel="noreferrer">Created By Deerflow</a></footer></div></section>{showPatientForm && <div className="modal-backdrop" role="presentation"><form className="modal-card" onSubmit={(event) => { event.preventDefault(); void handleCreatePatient(); }}><div className="modal-heading"><div><p className="eyebrow">NOVO CADASTRO</p><h2>Cadastrar paciente</h2></div><button type="button" className="dots" onClick={() => setShowPatientForm(false)} aria-label="Fechar">×</button></div><label>Nome completo<input required value={patientForm.name} onChange={(event) => setPatientForm({ ...patientForm, name: event.target.value })} /></label><label>Telefone<input required value={patientForm.phone} onChange={(event) => setPatientForm({ ...patientForm, phone: event.target.value })} placeholder="(11) 99999-9999" /></label><label>Observações<textarea value={patientForm.notes} onChange={(event) => setPatientForm({ ...patientForm, notes: event.target.value })} /></label>{!clinicId && <p className="form-warning">Configure NEXT_PUBLIC_CLINIC_ID para salvar no banco.</p>}<div className="modal-actions"><button type="button" className="ghost-button" onClick={() => setShowPatientForm(false)}>Cancelar</button><button type="submit" className="primary-button" disabled={savingPatient || !clinicId}>{savingPatient ? 'Salvando...' : 'Salvar paciente'}</button></div></form></div>}{showConversationForm && <ConversationModal form={conversationForm} setForm={setConversationForm} saving={savingConversation} error={conversationError} onClose={() => { setShowConversationForm(false); setConversationError(''); }} onSubmit={() => { void handleCreateConversation(); }} />}{recordPatient && <RecordModal patient={recordPatient} onClose={() => setRecordPatient(null)} />}{showHoursForm && <HoursModal form={hoursForm} setForm={setHoursForm} onClose={() => setShowHoursForm(false)} onSubmit={handleSaveHours} />}</main>;
}
