...
function SectionScreen({ section, data, appointments, conversations, checkedIn, onCheckIn, selectedConversationId, onSelectConversation, aiEnabled, onToggleAi, aiSaving, suggestion, suggesting, onSuggestReply, evolutionStatus, patientRecords, team, currentUser, onNewPatient, onNewAppointment, onNewConversation, conversationFilter, onConversationFilterChange, unreadConversationIds, replyText, onReplyTextChange, sendingReply, sendError, onSendReply }: { section: string; data: DashboardData; appointments: Appointment[]; conversations: Conversation[]; checkedIn: string[]; onCheckIn: (id: string) => void; selectedConversationId?: string; onSelectConversation: (id: string) => void; aiEnabled: boolean; onToggleAi: () => void; aiSaving: boolean; suggestion: string; suggesting: boolean; onSuggestReply: (conversation?: Conversation) => void; evolutionStatus?: IntegrationStatus; patientRecords: PatientSummary[]; team: TeamMember[]; currentUser: SessionUser | null; onNewPatient: () => void; onNewAppointment?: () => void; onNewConversation: () => void; conversationFilter: ConversationFilter; onConversationFilterChange: (value: ConversationFilter) => void; unreadConversationIds: Set<string>; replyText: string; onReplyTextChange: (value: string) => void; sendingReply: boolean; sendError: string; onSendReply: () => void }) {
  ...
  if (section === 'Equipe') return <><SectionIntro eyebrow="GESTÃO · PROFISSIONAIS" title="Equipe clínica" description="Veja quem está online e como as agendas estão distribuídas." action="Adicionar profissional" /><div className="team-grid">{people.map((person, index) => { const tone = person.role === 'ADMIN' || person.role === 'MASTER' ? 'lime' : ['blue', 'violet', 'green'][index % 3]; return <div className="panel team-card" key={person.id || person.name}><div className={`avatar avatar-${tone}`}>{initials(person.name)}</div><div><h2>{person.name}</h2><p>{person.specialty || roleLabel(person.role)}</p></div><span className="online-indicator" /></div>; })}{!people.length && <div className="empty-state">Nenhum profissional cadastrado.</div>}</div></>;
