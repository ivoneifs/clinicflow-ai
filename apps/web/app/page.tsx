...
function ConversationList({ conversations, selectedId, onSelect }: { conversations: Conversation[]; selectedId?: string; onSelect: (id: string) => void }) {
  return <div className="conversation-list">{conversations.length === 0 ? <div className="empty-state">Nenhuma conversa recebida hoje.</div> : conversations.map((conversation, index) => <button className={`conversation ${conversation.id === selectedId ? 'conversation-selected' : ''}`} key={conversation.id} onClick={() => onSelect(conversation.id)}><div className={`avatar avatar-${['coral', 'violet', 'green'][index % 3]}`}>{initials(conversation.patient?.name || 'Paciente')}</div><div className="conversation-copy"><div><strong>{conversation.patient?.name || 'Paciente'}</strong>{index < 2 && <span className="unread-dot" />}</div><p>{conversation.messageText || 'Mensagem sem texto'}</p></div><time>{relativeTime(conversation.createdAt)}</time></button>)}</div>;
}

function Overview({ data, appointments, conversations, loading, displayDate, checkedIn, onCheckIn }: { data: DashboardData; appointments: Appointment[]; conversations: Conversation[]; loading: boolean; displayDate: string; checkedIn: string[]; onCheckIn: (id: string) => void }) {
  return <><div className="welcome-row"><div><p className="eyebrow">{displayDate.toUpperCase()}</p><h1>Bom dia, Mariana <span>✦</span></h1><p className="subheading">Aqui está o pulso da sua clínica hoje.</p></div><button className="primary-button"><Plus size={17} />Novo agendamento</button></div>
    ...
    <div className="dashboard-grid"><div className="panel agenda-panel"><div className="panel-header"><div><h2>Agenda de hoje</h2><p>{displayDate.split(' de ')[0]} <span>·</span> {data.appointments.length} consultas</p></div><button className="ghost-button">Ver agenda completa <ArrowUpRight size={15} /></button></div><AppointmentRows appointments={appointments} checkedIn={checkedIn} onCheckIn={onCheckIn} /></div><div className="panel inbox-panel"><div className="panel-header"><div><h2>Atendimento</h2><p>WhatsApp <span className="live-dot" /> ao vivo</p></div><button className="circle-add" aria-label="Nova conversa"><Plus size={17} /></button></div><div className="inbox-tabs"><button className="tab-active">Todas <b>{data.conversations.length}</b></button><button>Não lidas <b>{Math.min(data.conversations.length, 4)}</b></button></div><ConversationList conversations={conversations} onSelect={() => undefined} /><button className="inbox-footer">Abrir central de atendimento <ArrowUpRight size={15} /></button></div></div>
...
function SectionScreen({ section, data, appointments, conversations, checkedIn, onCheckIn, selectedConversationId, onSelectConversation, aiEnabled, onToggleAi, suggestion, suggesting, onSuggestReply, evolutionStatus, patientRecords, team, onNewPatient }: { section: string; data: DashboardData; appointments: Appointment[]; conversations: Conversation[]; checkedIn: string[]; onCheckIn: (id: string) => void; selectedConversationId?: string; onSelectConversation: (id: string) => void; aiEnabled: boolean; onToggleAi: () => void; suggestion: string; suggesting: boolean; onSuggestReply: (conversation?: Conversation) => void; evolutionStatus?: IntegrationStatus; patientRecords: PatientSummary[]; team: TeamMember[]; onNewPatient: () => void }) {
  ...
  if (section === 'Agenda') return <><SectionIntro eyebrow="OPERAÇÃO · AGENDA" title="Agenda" description="Organize o dia da equipe com uma visão simples e viva." action="Novo agendamento" /><div className="section-grid"><div className="panel"><div className="panel-header"><div><h2>Quinta-feira, hoje</h2><p>{appointments.length} consultas programadas</p></div><div className="date-chip"><CalendarDays size={14} />04 set</div></div><AppointmentRows appointments={appointments} checkedIn={checkedIn} onCheckIn={onCheckIn} /></div><div className="panel insight-panel"><p className="eyebrow">RESUMO DO DIA</p><div className="big-number">{data.waiting}</div><h2>paciente aguardando</h2><p>O próximo horário livre aparece assim que um profissional libera a agenda.</p><div className="mini-stat"><span>Confirmadas</span><strong>{appointments.filter((item) => item.status === 'SCHEDULED').length}</strong></div><div className="mini-stat"><span>Pendentes</span><strong>{appointments.filter((item) => item.status === 'PENDING_CONFIRMATION').length}</strong></div></div></div></>;
