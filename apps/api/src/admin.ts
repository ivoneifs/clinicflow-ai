...
export async function registerAdminRoutes(app: FastifyInstance) {
  ...
  app.get('/api/v1/admin/users', async (request, reply) => { if (!requireMaster(request, reply)) return; const query = z.object({ clinicId: z.string().optional(), includeInactive: z.coerce.boolean().default(false) }).parse(request.query); return prisma.user.findMany({ where: { ...(query.clinicId ? { clinicId: query.clinicId } : {}), ...(query.includeInactive ? {} : { active: true }) }, select: { id: true, clinicId: true, name: true, email: true, role: true, specialty: true, active: true, createdAt: true, clinic: { select: { name: true } } }, orderBy: { createdAt: 'desc' }, take: 200 }); });
