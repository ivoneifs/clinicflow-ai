...
export async function registerAdminRoutes(app: FastifyInstance) {
  ...
  app.post('/api/v1/admin/users', async (request, reply) => { if (!requireMaster(request, reply)) return; const body = userInput.parse(request.body); if (!body.password) return reply.code(400).send({ error: 'Defina uma senha de pelo menos 12 caracteres para o profissional.' }); try { return await prisma.user.create({ data: { clinicId: body.clinicId, name: body.name, email: body.email, role: body.role, active: body.active, specialty: body.specialty || null, passwordHash: await hashUserPassword(body.password) }, select: { id: true, clinicId: true, name: true, email: true, role: true, specialty: true, active: true } }); } catch { return reply.code(409).send({ error: 'E-mail já está em uso nesta clínica.' }); } });
  app.patch('/api/v1/admin/users/:id', async (request, reply) => { if (!requireMaster(request, reply)) return; const body = userInput.partial().omit({ clinicId: true }).parse(request.body); const params = z.object({ id: z.string() }).parse(request.params); try { const data = { name: body.name, email: body.email, role: body.role, active: body.active, specialty: body.specialty === '' ? null : body.specialty, ...(body.password ? { passwordHash: await hashUserPassword(body.password) } : {}) }; return await prisma.user.update({ where: { id: params.id }, data, select: { id: true, clinicId: true, name: true, email: true, role: true, specialty: true, active: true } }); } catch { return reply.code(404).send({ error: 'Profissional não encontrado.' }); } });
