...
const userInput = z.object({ clinicId: z.string().min(1), name: z.string().trim().min(2), email: z.string().email(), role: z.enum(['ADMIN', 'DOCTOR', 'ATTENDANT']), specialty: z.string().trim().optional().or(z.literal('')), active: z.boolean().default(true) });
...
export async function registerAdminRoutes(app: FastifyInstance) {
  ...
  app.post('/api/v1/admin/users', async (request, reply) => { if (!requireMaster(request, reply)) return; const body = userInput.parse(request.body); try { return await prisma.user.create({ data: { ...body, specialty: body.specialty || null, passwordHash: 'MASTER_MANAGED' }, select: { id: true, clinicId: true, name: true, email: true, role: true, specialty: true, active: true } }); } catch { return reply.code(409).send({ error: 'E-mail já está em uso nesta clínica.' }); } });
  app.patch('/api/v1/admin/users/:id', async (request, reply) => { if (!requireMaster(request, reply)) return; const body = userInput.partial().omit({ clinicId: true }).parse(request.body); const params = z.object({ id: z.string() }).parse(request.params); try { return await prisma.user.update({ where: { id: params.id }, data: { ...body, specialty: body.specialty === '' ? null : body.specialty }, select: { id: true, clinicId: true, name: true, email: true, role: true, specialty: true, active: true } }); } catch { return reply.code(404).send({ error: 'Profissional não encontrado.' }); } });
