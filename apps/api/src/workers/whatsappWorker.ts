...
export const whatsappWorker = new Worker<IncomingJob>('clinicflow-whatsapp', async (job) => {
  ...
  if (env.N8N_AI_WEBHOOK_URL) {
    const response = await fetch(env.N8N_AI_WEBHOOK_URL, { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify(job.data) });
  ...
  return { forwardedToN8n: Boolean(env.N8N_AI_WEBHOOK_URL) };
